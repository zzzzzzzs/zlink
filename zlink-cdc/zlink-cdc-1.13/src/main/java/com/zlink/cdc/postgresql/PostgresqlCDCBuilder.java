package com.zlink.cdc.postgresql;
 ...
 * @date 2022/12/8
 */
public class PostgresqlCDCBuilder {

    private static Logger logger = LoggerFactory.getLogger(PostgresqlCDCBuilder.class);
...
//        String url = config.getSinkUrl().substring(0, config.getSinkUrl().lastIndexOf("/")) + "/" + config.getSinkTable().getSchema();
        String tableName = config.getSinkTable().getSchema() + "." + config.getSinkTable().getName();
        StringBuilder sb = new StringBuilder();
        sb
                .append("CREATE TABLE ")
        ...
        sb.append(") with ( \n")
                ...
                .append(" 'url' = '").append(config.getSinkUrl()).append("',\n")
